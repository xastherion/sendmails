...
def read_config():
    try:
        with open(cfg_file, "r") as file:
            ...
            # Sicherstellen, dass es mindestens 5 Zeilen gibt
            if len(lines) < 5:
                lines += ["\n"] * (5 - len(lines))  # Füge leere Zeilen hinzu, falls weniger als 5 Zeilen
            absender = lines[0].strip().strip('"')
            betreff = lines[1].strip()
            kontaktlistenpfad = lines[2].strip()
            emailinhaltpfad = lines[3].strip()
            verzogerung = lines[4].strip() if len(lines) > 4 else "0"  # Standardwert für Verzögerung
        return absender, betreff, kontaktlistenpfad, emailinhaltpfad, verzogerung
    except FileNotFoundError:
        # Falls die Datei nicht existiert, mit Standardwerten starten
        return "", "", "", "", "0"
...
def save_config(absender, betreff, kontaktlistenpfad, emailinhaltpfad, verzogerung):
    try:
        ...
        messagebox.showinfo("Erfolg", "Die Einstellungen wurden erfolgreich gespeichert.")
    except Exception as e:
        messagebox.showerror("Fehler", f"Die Einstellungen konnten nicht gespeichert werden: {e}")
...
def create_gui():
    ...
    def choose_file(entry_field):
        ...
        if filepath:
            filename = os.path.basename(filepath)  # Nur den Dateinamen ohne Pfad verwenden
    ...
    # Speichern, Senden und Beenden Buttons
